...
def main():
    ...
    if a.mrc_nic:
        ...
        if nic_src:
            ...
            tw_src = os.path.join(os.path.dirname(nic_src), "mrc-probe")
    ...
    for name, r in nodes.items():
        ...
        if a.mrc_nic and r["role"] == "gpu":
            ...
            out.append("        - sh -c 'install -m 0755 /opt/mrc-probe.bin /usr/local/bin/mrc-probe'")
            out.append("        - sh -c 'nohup /usr/local/bin/mrc-probe responder "
                       ">/var/log/mrc-probe.log 2>&1 &'")
